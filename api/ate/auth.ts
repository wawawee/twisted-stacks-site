...
function memberCookie(member: string, clear = false) {
  ...
  const parts = [
    `ate_member=${clear ? "" : encodeURIComponent(member)}`,
    "Path=/",
    "SameSite=Lax",
    `Max-Age=${maxAge}`,
  ];
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "GET") {
    ...
    res.status(200).json({
      authenticated: ok,
      member: ok ? cookies.get("ate_member") || null : null,
    });
    return;
  }
  ...
  if (req.method === "POST") {
    ...
    res.setHeader("Set-Cookie", [
      `ate_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_MAX_AGE_SEC}${secureFlag}`,
      memberCookie(member),
    ]);
  ...
  if (req.method === "DELETE") {
    ...
    const secureFlag = secure ? "; Secure" : "";
    res.setHeader("Set-Cookie", [
      `ate_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0${secureFlag}`,
      memberCookie("", true),
    ]);
