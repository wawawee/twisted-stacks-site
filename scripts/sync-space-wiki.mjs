#!/usr/bin/env node
/**
 * Sync SPACE project surface: wiki/, TASKLIST.md, docs/HISTORY.md → space-wiki/
 */
...
const OUT_DIR = path.join(ROOT, "space-wiki");

const GITHUB = { owner: "finasteos", repo: "SPACEinSPACE", branch: "main" };

const TOPIC_LABELS = {
  blender: "Blender ambassador",
  unity: "Unity & Godot",
  commons: "Commons & avatars",
  charter: "Charter & guest rights",
  sessions: "Sessions & signup",
  architecture: "Architecture",
  misc: "Övrigt",
};

function localRepoRoot() {
  const fromEnv = process.env.SPACE_REPO_LOCAL?.trim();
  if (fromEnv) return path.resolve(fromEnv);
  return path.resolve(ROOT, "../SPACEinSPACE");
}
...
async function fetchRepoFile(repoPath) {
  ...
  if (!res.ok) {
    throw new Error(
      `${repoPath}: ${res.status}${token ? "" : " (private repo? set GITHUB_TOKEN for Vercel build)"}`,
    );
  }
  return res.text();
}
...
async function main() {
  ...
  if (fromGitHub) {
    ...
    historyMd = await fetchRepoFile("docs/HISTORY.md").catch(
      () => "# History\n\n## Milestones\n\n- 2026-07-22 — finasteos/SPACEinSPACE fork + Unity/Godot ambassadors\n",
    );
  } else {
    tasklistMd = await fs.readFile(path.join(repoRoot, "TASKLIST.md"), "utf8");
    const historyPath = path.join(repoRoot, "docs/HISTORY.md");
    if (await pathExists(historyPath)) {
      historyMd = await fs.readFile(historyPath, "utf8");
    } else {
      historyMd =
        "# History\n\n## Milestones\n\n- 2026-07-22 — finasteos/SPACEinSPACE fork + Unity/Godot ambassadors\n- 2026-07-23 — twistedstacks.com /space collab room\n";
    }
  }
