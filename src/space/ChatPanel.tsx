...
export default function ChatPanel({ memberId }: { memberId: string | null }) {
  ...
  async function fetchMessages() {
    try {
      const res = await fetch("/api/space/chat?limit=80");
  ...
  return (
    <div className="space-chat">
      ...
      <div className="space-chat-feed" ref={scrollRef} aria-live="polite">
        {loading && messages.length === 0 ? (
          <p className="space-loading">Laddar meddelanden…</p>
        ) : messages.length === 0 ? (
          <p className="space-chat-empty">Inga meddelanden än. Skriv det första!</p>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`space-chat-msg${mine ? " mine" : ""}`}>
                <div className="space-chat-msg-head">
                  <span className="space-chat-avatar" style={{ borderColor: color }}>
                    {label.charAt(0)}
                  </span>
                ...
                <div className="space-chat-msg-body">{msg.body}</div>
              </div>
            );
          })
        )}
      ...
      <form className="space-chat-compose" onSubmit={send}>
        <textarea
          value={draft}
          ...
          disabled={sending}
        />
        <div className="space-chat-compose-actions">
          <span className="space-chat-hint">{draft.length}/2000</span>
