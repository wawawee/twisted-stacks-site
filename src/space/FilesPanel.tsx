...
export default function FilesPanel() {
  ...
  const loadFiles = useCallback(async () => {
    try {
      const res = await fetch("/api/space/files");
  ...
  async function uploadFile(file: File) {
    ...
    setError("");
    try {
      const prep = await fetch("/api/space/files", {
        method: "POST",
  ...
  return (
    <div className="space-files">
      <header className="space-files-header">
        <div>
          <h2>Delade filer</h2>
          <p>PDF, bilder, ljud, zip, markdown — max 25 MB. Större filer? Maila teamet.</p>
        </div>
        <div className="space-files-actions">
          <button type="button" className="space-btn" onClick={loadFiles} disabled={loading || uploading}>
            Uppdatera
          </button>
          <label className="space-btn space-btn-primary space-file-picker">
      ...
      {loading && files.length === 0 ? (
        <p className="space-loading">Laddar filer…</p>
      ) : files.length === 0 ? (
        <p className="space-files-empty">Inga filer ännu.</p>
      ) : (
        <ul className="space-files-list">
          {files.map((file) => (
            <li key={file.id} className="space-files-item">
              <div className="space-files-meta">
              ...
              {file.downloadUrl ? (
                <a href={file.downloadUrl} className="space-btn" download={file.name} target="_blank" rel="noreferrer">
                  Ladda ner
                </a>
              ) : (
                <span className="space-chat-hint">Länk saknas</span>
              )}
            </li>
          ))}
        </ul>
