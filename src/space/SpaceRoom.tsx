...
import SpaceMobileNav, { type SpaceMobileNavId } from "./SpaceMobileNav";
...
function LoginPanel({ onSuccess }: { onSuccess: (member: string) => void }) {
  ...
  return (
    <div className={`space-root${isDark ? " theme-dark" : ""}`}>
      <div className="room-login">
        <form className="room-login-card" onSubmit={submit}>
          <h1>SPACEinSPACE</h1>
          <p className="room-login-lede">
            Colab for people and agents — sessions, Blender worlds, Unity, Commons.
          </p>

          <div className="room-login-field">
            <label htmlFor="space-member">Vem är du?</label>
            <select
              id="space-member"
              value={member}
          ...
          <div className="room-login-field">
            <label htmlFor="space-password">Lösenord</label>
            <input
              id="space-password"
              type="password"
              value={password}
...
export default function SpaceRoom() {
  ...
  const loadProject = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/space/project");
      const data = await res.json();
  ...
  async function logout() {
    await fetch("/api/space/auth", { method: "DELETE" });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setMdLoading(true);
    fetch(`/api/space/wiki?page=${encodeURIComponent(slug)}`)
      .then(async (res) => {
        const data = await res.json();
  ...
  const mobileNavActive: SpaceMobileNavId =
    selection?.slug === "chat"
      ? "chat"
  ...
  function handleMobileNav(id: SpaceMobileNavId) {
    if (id === "home") {
      setSelection(null);
      setActiveId(null);
      return;
  ...
  const panelContent =
    selection?.kind === "overview" && project ? (
      ...
      <FilesPanel />
    ) : selection?.slug === "sessions-signup" ? (
      <SessionsPanel />
    ) : (
      <DetailPanel
        selection={selection}
  ...
  return (
    <div className={`space-root space-mobile${isDark ? " theme-dark" : ""}`}>
      ...
      <div className="room-layout">
        ...
        <main className="room-main">
          ...
          {projectLoading ? (
            ...
            isMobile && !selection ? (
              <div className="space-mobile-home">
                <OverviewPanel
                  tasklist={project.tasklist}
                  history={project.history}
                  syncedAt={project.manifest.syncedAt}
      ...
      {isMobile ? <SpaceMobileNav active={mobileNavActive} onSelect={handleMobileNav} /> : null}
    </div>
  );
}
